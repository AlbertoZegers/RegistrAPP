  ...
  constructor(private sqlite: SQLite ) { 
    ...
    crearTablas(){
        ...
        .then((db: SQLiteObject) => {
          ...
          db.executeSql('CREATE TABLE IF NOT EXISTS USUARIO(USUARIO VARCHAR(32), CORREO VARCHAR(32), PASSWORD VARCHAR(32), NOMBRE VARCHAR(32), APELLIDO VARCHAR(32))', [])
    ...
    almacenarUsuario(usuario: string, correo: string, password: string, nombre: string, apellido: string) {
      this.sqlite.create({
        name: 'data.db',
        location: 'default'
      })
      .then((db: SQLiteObject) => {
        ...
        db.executeSql('INSERT INTO USUARIO VALUES(?,?,?,?,?)', [usuario, correo, password, nombre, apellido])
